...
/*
 * [View를 만드는 방법]
 * - AbstractView추상클래스를 상속 받는다
 * - @Component("bean의 이름 지정")
 * - ModelAndView 객체에 담은 모델은 Map 객체로 전달된다
 * - servlet=context.xml에 BeanNameViewResolver설정
 */
@Component("fileDownView")//bean의 이름 지정하기
public class FileDownView extends AbstractView{

	@Override
	protected void renderMergedOutputModel(Map<String, Object> model, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
			//ModelAndView 객체에 담았던 값이 Map객체에 담겨서 전달된다
			...
			//ServletContext 객체의 참조값(jsp페이지에서는 기본객체)
